...
| | |
|---|---|
| 🧩 **轻量** | 仅 **~33.2M** 参数 — 比 LLM 基时序模型小一个数量级 |
| 🔮 **长程上下文** | 支持最长 **1024 时间步** 的零样本泛化预测 |
| 📊 **性能卓越** | 横跨 **3 个基准、8 项任务、32 组对比实验**，表现突出 |

### 🏆 评测成绩

**输入长度 = 512（3 个基准 × 8 项任务）**
...
**全量对比：128 / 256 / 512 / 1024 × 8 项任务 = 32 组实验**
...
> 在与 Google TimesFM 2.5 和 Amazon Chronos 2 的对比中，MeteoTime 以 **不到 1/6 的参数量**（33.2M vs 200M）取得了相当甚至更优的结果。

## 模型规格

- 约 33.2M 参数的 Decoder-only Transformer
- 最大输入 1024 小时，Patch 大小 32
- 单次前向传播预测未来 64 小时
